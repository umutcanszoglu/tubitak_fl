...
class DoubleCircularNotchedButton extends NotchedShape {
  ...
  Path getOuterPath(Rect host, Rect? guest) {
    ...
    final List<Offset> px = List.generate(6, (index) => Offset.zero);
    ...
    for (int i = 0; i < px.length; i += 1) {
      px[i] += Offset(0 + (notchRadius + 12), 0);
    }

    final List<Offset> py = List.generate(6, (index) => Offset.zero);
    ...
    for (int i = 0; i < py.length; i += 1) {
      py[i] += Offset(host.width - (notchRadius + 12), 0);
    }
